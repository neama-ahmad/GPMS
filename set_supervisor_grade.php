...
if($_SERVER["REQUEST_METHOD"] == "POST"){
 ...
 if(isset($_POST["btn-add"])){
    ...
    $supervisor_mark = $_POST['supervisor_mark'];
    $supervisor_id = $_POST['supervisor_id'];

    //check supervisor didn't submit grade before for this student
    ...
    if($count < 1 ){
        //supervisor submit student's grade
        $Submit_Grade = mysqli_query($db, "INSERT INTO grades (mark, recorder, student_id) VALUES ('$supervisor_mark', 'supervisor', '$student_id')");
        $grade_id = mysqli_insert_id($db);
        if($grade_id){
            $save_recorder_id = mysqli_query($db, "INSERT INTO grade_recorder (recorder_id, grade_id) VALUES ('$supervisor_id', '$grade_id')");
            header('Location:supervisor_grade.php');
        }
    }
    else{
    //if supervisor submit grade for this student before just Update the grade
    $Update_Grade = mysqli_query($db,"UPDATE grades SET  mark  = '$supervisor_mark' WHERE student_id =  '$student_id' AND recorder = 'supervisor'");
    header('Location:supervisor_grade.php');
    }
 ...
 //Confirm student's grade (اعتماد الدرجة)
  if(isset($_POST["btn-ok"])){
    ...
    $Update_grade = mysqli_query($db,"UPDATE grades SET  mark  = '$supervisor_mark' WHERE student_id =  '$student_id' AND recorder = 'supervisor' ");
    $Confirm_grade = mysqli_query($db,"UPDATE grades SET  confirm = 1 WHERE student_id = '$student_id' AND recorder = 'supervisor' ");
    header('Location:supervisor_grade.php');
   } 
...
<head>
    ...
    <body>
        ...
        <?php
          echo'
          <a href="supervisor_grade.php" class="Toback">&#8594;</a>';
        ?>
           ...
           <form  method="POST" >
           <?php 
                $query2 = mysqli_query($db,"SELECT * FROM group_members WHERE user_id = '$student_id' ");
                ...
                <table id="rowTable">
                    ...
                    <tr>
                        ...
                        if($count > 0){
                            ...
                            echo'
                            <td>
                            <input type="number" name="supervisor_mark" max="55" min="0" class="forminput number" placeholdeer="ادخل درجة الطالب" value="'.$mark.'" required></td>
                            <td>
                            <input type="hidden" name="supervisor_id" class="forminput" value="'.$userID.'" readonly>
                            ...
                            echo'
                            <td>
                            <input type="number" name="supervisor_mark"  max="45"  min="0" class="forminput number" placeholdeer="ادخل درجة الطالب" required></td>
                            <td>
                            <input type="hidden" name="supervisor_id" class="forminput" value="'.$userID.'" readonly>
                            <input type="submit" id="btn-add" name="btn-add" class="button btn-send" value="حفظ">
