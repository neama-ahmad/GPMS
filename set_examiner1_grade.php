...
if($_SERVER["REQUEST_METHOD"] == "POST"){
 ...
 if(isset($_POST["btn-add"])){
    ...
    //check examiner1 didn't submit grade before for this student
    $hasGrade = mysqli_query($db, "SELECT * FROM grades WHERE student_id = '$student_id'  AND recorder = 'examiner1' ");
    $count = mysqli_num_rows($hasGrade);
    if($count < 1 ){
        //examiner1 submit student's grade
        $Submit_Grade = mysqli_query($db, "INSERT INTO grades (mark, recorder, student_id) VALUES ('$examiner_mark', 'examiner1', '$student_id')");
     ...
     //if examiner1 submit grade for this student before just Update the grade
     $Update_Grade = mysqli_query($db,"UPDATE grades SET  mark  = '$examiner_mark' WHERE student_id =  '$student_id' AND recorder = 'examiner1'");
     header('Location:examiner_grade.php');
    }
...
if(isset($_POST["btn-ok"])){
    ...
    $Update_grade = mysqli_query($db,"UPDATE grades SET  mark  = '$examiner_mark' WHERE student_id =  '$student_id' AND recorder = 'examiner1' ");
    $Confirm_grade = mysqli_query($db,"UPDATE grades SET  confirm = 1 WHERE student_id = '$student_id' AND recorder = 'examiner1' ");
    header('Location:examiner_grade.php');
   } 
}
